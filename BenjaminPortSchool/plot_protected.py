...
def plot_protected(X, Y, Y_pred=None, Z=None, W=None, Y_W=None, fig=None):
	...
	selec = np.logical_and(Z == 0, np.logical_and(Y == 0, Y_pred >= 0.5))
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('skyblue', 0), edgecolors=tango_color('scarletred', 2), marker='o')
	...
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('scarletred', 0), edgecolors=tango_color('skyblue', 2), marker='o')
	# Plot non-protected people who have a positive label and are
	# predicted as positive label
	selec = np.logical_and(Z == 0, np.logical_and(Y == 1, Y_pred >= 0.5))
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('scarletred', 0), edgecolors=tango_color('scarletred', 2), marker='o')
	...
	# predicted as positive label
	selec = np.logical_and(Z == 1, np.logical_and(Y == 0, Y_pred >= 0.5))
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('skyblue', 0), edgecolors=tango_color('scarletred', 2), marker='s')
	# Plot protected people who have a positive label but are
	# predicted as negative label
	selec = np.logical_and(Z == 1, np.logical_and(Y == 1, Y_pred < 0.5))
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('scarletred', 0), edgecolors=tango_color('skyblue', 2), marker='s')
	# Plot protected people who have a positive label and are
	# predicted as positive label
	selec = np.logical_and(Z == 1, np.logical_and(Y == 1, Y_pred >= 0.5))
	ax.scatter(X[selec, 0], X[selec, 1], c=tango_color('scarletred', 0), edgecolors=tango_color('scarletred', 2), marker='s')

	# Plot prototypes
	if(W is not None):
		# Plot prototypes for the negative label
		ax.scatter(W[Y_W == 0, 0], W[Y_W == 0, 1], c=tango_color('skyblue', 1), edgecolors=tango_color('skyblue', 2), linewidths=2, s=150, marker='D')
		# Plot prototypes for the positive label
		ax.scatter(W[Y_W == 1, 0], W[Y_W == 1, 1], c=tango_color('scarletred', 1), edgecolors=tango_color('scarletred', 2), linewidths=2, s=150, marker='D')
