...
#		x[9] = float(data_row[29])
		...
		# Gender
		Z.append(1 if data_row[1] == 'F' else 0)
		# age
...
print('{} percent of people in the protected group have a good grade.'.format(np.sum(np.logical_and(Z == 1, Y == 1)) / np.sum(Z == 1)))
print('{} percent of people outside the protected group have a good grade.'.format(np.sum(np.logical_and(Z == 0, Y == 1)) / np.sum(Z == 0)))
